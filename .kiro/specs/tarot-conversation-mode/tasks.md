...
- [x] 1. Set up Gemini server-side handler and API route
  - [x] 1.1 Create `src/components/Tarot/services/geminiHandler.js` with the Gemini API handler logic
  ...
  - [x] 1.2 Create `api/gemini.js` as thin Vercel entry point
...
- [x] 2. Create Gemini frontend client
  - [x] 2.1 Create `src/components/Tarot/services/geminiClient.js`
...
- [x] 5. Create Conversation Mode UI components
  - [x] 5.1 Create `src/components/Tarot/ConversationInput.jsx`
  ...
  - [x] 5.2 Create `src/components/Tarot/ConversationTurn.jsx`
  ...
  - [x] 5.3 Create `src/components/Tarot/ConversationHistory.jsx`
    - Scrollable container rendering all turns via ConversationTurn
    - Auto-scroll to bottom when new turn added
    - _Requirements: 2.1, 6.1, 6.2_

  - [x] 5.4 Create `src/components/Tarot/LoadingIndicator.jsx`
    - Spinner and "Interpreting your cards..." message
    - _Requirements: 2.5_

  - [x] 5.5 Create `src/components/Tarot/ConversationMode.jsx` (main page component)
...
- [x] 6. Add routing and entry point button
  - [x] 6.1 Add `/conversation` route to `src/App.jsx`
    - Lazy-load ConversationMode component
    - Add route with Suspense wrapper
    - _Requirements: 1.3_

  - [x] 6.2 Add "Conversation Mode" button to existing Tarot page
    - Add button with tooltip text to Tarot.jsx
    - Use react-router-dom Link or navigate to /conversation on click
    - _Requirements: 1.1, 1.2_

- [x] 7. Add SCSS styles for Conversation Mode
  - [x] 7.1 Add conversation mode styles to `src/components/Tarot/Tarot.module.scss`
...
- [x] 8. Update Vercel and environment configuration
  - [x] 8.1 Update `vercel.json` to support the API route alongside the SPA rewrite
    - Ensure `/api/gemini` is handled as a serverless function
    - Keep existing SPA rewrite for all other routes
    - _Requirements: 5.5_

  - [x] 8.2 Create `.env.local.example` documenting required environment variables
